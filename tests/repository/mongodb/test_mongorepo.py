...
def test_repository_list_without_parameters(docker_setup, mg_data, mg_database):
    ...
    assert {r.code for r in repo_rooms} == {r["code"] for r in mg_data}


def test_repository_list_with_code_equal_filter(docker_setup, mg_data, mg_database):
    repo = mongorepo.MongoRepo(docker_setup["mongo"])
...
def test_repository_list_with_price_equal_filter(docker_setup, mg_data, mg_database):
    repo = mongorepo.MongoRepo(docker_setup["mongo"])
...
def test_repository_list_with_price_between_filter(docker_setup, mg_data, mg_database):
    repo = mongorepo.MongoRepo(docker_setup["mongo"])
...
def test_repository_list_with_price_as_string(docker_setup, mg_data, mg_database):
    repo = mongorepo.MongoRepo(docker_setup["mongo"])
    repo_rooms = repo.list(filters={"price__lt": "60"})
    assert len(repo_rooms) == 2
    assert set([r.code for r in repo_rooms]) == {
